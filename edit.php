...
// select the id with the user_id in the post
$get_post = mysqli_query($conn, "SELECT * FROM post WHERE id = '$user_id'");
// fetch the post data that match with the user_id
while ($row_edit = mysqli_fetch_array($get_post)) {
    $title = $row_edit['title'];
    $description = $row_edit['description'];
    $content = $row_edit['content'];
    $author = $row_edit['author'];
    $image = $row_edit['image_url'];
}
// if submit button is clicked get the new data from the form and update the data in the database
if (isset($_POST['submit'])) {
    ...
    // update query
    $upd = mysqli_prepare($conn, "UPDATE post SET title=?, description=?, content=?, author=?, image_url=? WHERE id=?");
    mysqli_stmt_bind_param($upd, "sssssi", $new_title, $new_description, $new_content, $new_author, $new_image, $user_id);
...
<body>
    ...
    <div class='container'>
        <form method="POST" data-aos='zoom-out' data-aos-duration='3500'>
            <h1>Update</h1>
            <input type="hidden" name="user_id" value="<?php echo $user_id; ?>">
            <div class="card">
                <label for="title">Title</label>
                <input type="text" name="new_title" required id="title" placeholder="Enter Title"
                    value="<?php echo $title; ?>">
            </div>
            <div class="card">
                <label for="author">Author</label>
                <input value="<?php echo $author; ?>" type="text" name="new_author" required id="author"
                    placeholder="Enter Author">
            </div>
            <div class="card">
                <label for="description">Description</label>
                <input value="<?php echo $description; ?>" type="text" name="new_description" required id="description"
                    placeholder="Enter Description">
            </div>
            <div class="card">
                <label for="content">Content</label>
                <input value="<?php echo $content; ?>" type="text" name="new_content" required id="content"
                    placeholder="Enter Content">
            </div>
            <div class="card">
                <label for="image">Image Url</label>
                <input value="<?php echo $image; ?>" type="text" name="new_image" required id="image"
